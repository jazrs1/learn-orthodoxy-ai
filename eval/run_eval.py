...
Environment:
    INTERNAL_API_KEY / ORTHODOX_API_KEY  shared secret sent as X-Internal-Key (required)
...
Do not reward length. Do not penalise a different language if the meaning is right.
...
def judge_answer(client: Any, model: str, item: Dict[str, Any], answer: str) -> Tuple[Optional[int], str]:
    ...
    try:
        ...
        return None, f"judge error: {exc!r}"[:300]
...
def main() -> int:
    ...
    summary: Dict[str, Any] = {
        ...
        "latency_s_mean": mean([r["latency_s"] for r in records if r["latency_s"]]),
    ...
    print(f"retrieval   recall@{args.k} {fmt(a['recall_at_k'])}  (+/-1 page {fmt(a['recall_at_k_tol1'])})  any-query {fmt(a['recall_any'])}  kept {fmt(a['recall_kept'])}  shown {fmt(a['recall_shown'])}  hit-rate {fmt(a['hit_rate_at_k'])}")
    ...
    print(f"latency mean {summary['latency_s_mean']:.1f}s   prompt tokens mean {summary['prompt_tokens_mean']:.0f}")
    print("\nby category:      n  answered  refused  clarif  recall@k   kept   judge")
    for category, row in summary["by_category"].items():
        print(
            ...
            f"{fmt(row['recall_at_k'])}  {fmt(row['recall_kept'])}  {fmt(row['judge_mean_all'], pct=False)}"
        )
