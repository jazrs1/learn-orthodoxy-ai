...
import { allowShareRequest, ipHash, loadShareableAnswer, saveSnapshot, validTimeZone } from "../../../lib/share-store";
...
// the answer's ID and its time zone. Limited per client (SHARE_RATE_LIMIT per SHARE_RATE_WINDOW_MINUTES).
export async function POST(request: Request) {
  const body = (await request.json().catch(() => ({}))) as { messageId?: unknown; timeZone?: unknown };
  const messageId = typeof body.messageId === "string" ? body.messageId.trim() : "";
  // The sharer's time zone, only to date the answer as their calendar has it (UI-034); not stored.
  ...
  try {
    ...
    const content = await loadShareableAnswer(db, sessionId, messageId, timeZone);
    if (!content) {
